...
fn main() {
    ...
    let f = tokio_core::reactor::Interval::new_at(
        ...
        .for_each(|_| {
                    ...
                    .map(|res| {
                        ...
                        let pin_futures = res.0
                            ...
                            .filter_map(|entry| {
                                ...
                                let meta: DtubeMetadata =
                                    match serde_json::from_str(&entry.json_metadata) {
                                        Ok(meta) => meta,
                                        Err(err) => {
                                            println!(
                                                "Couln't deserialize dtube meta: `{:?}` err: `{}`",
                                ...
                                let pin_futures: Vec<_> =
                                    [snaphash, spritehash, videohash, video480hash]
                                        .into_iter()
                                        .cloned()
                                        .filter_map(|hash| hash)
                                        .map(|hash| {
                                            println!("pinning: {:?}", hash);
                                            ipfs.pin_add(hash, true).map_err(Error::Ipfs)
                                        })
                                        .collect();
                                Some(pin_futures)
                            })
            ...
            futures::stream::iter_ok(name_futures).for_each(|_| Ok(()))
        });
    core.run(f).unwrap();
}
